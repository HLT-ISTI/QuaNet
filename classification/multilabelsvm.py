...
class MLSVC:
    ...
    def fit(self, X, y, **grid_search_params):
        ...
        if grid_search_params:
            ...
            # Grid search fails if the category prevalence is less than the parameter cv. In those cases we
        ...
        self.svms = Parallel(n_jobs=self.n_jobs)(delayed(self.svms[c].fit)(X,y[:,c]) for c,svm in enumerate(self.svms))
        self.training_time = time() - tini

        # for c,svm in enumerate(self.svms):
        #     if isinstance(svm, GridSearchCV):
        #         print('cat',c,svm.best_params_)
