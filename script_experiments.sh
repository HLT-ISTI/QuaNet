...
#	python3 learn_class.py $data --output $classmodel
	...
	python3 eval_quant.py $data $classmodel $quantmodelES $run -E --plotdir ../eval_plots_bound --results ../results/eval_results_bound.txt --result-note $run --include-bounds

	python3 learn_quant.py $data $classmodel --output $quantmodel --include-bounds
	python3 eval_quant.py $data $classmodel $quantmodel $run --plotdir ../eval_plots_bound --results ../results/eval_results_bound.txt --result-note $run --include-bounds --net-only

	python3 learn_quant.py $data $classmodel -E --output $quantmodelE --include-bounds
	python3 eval_quant.py $data $classmodel $quantmodelE $run -E --plotdir ../eval_plots_bound --results ../results/eval_results_bound.txt --result-note $run --include-bounds --net-only

	python3 learn_quant.py $data $classmodel --stats-layer --output $quantmodelS --include-bounds
	python3 eval_quant.py $data $classmodel $quantmodelS $run --plotdir ../eval_plots_bound --results ../results/eval_results_bound.txt --result-note $run --include-bounds --net-only
done
done
